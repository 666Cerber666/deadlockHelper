<template>
  <div v-if="characters" class="wrapper">
    <div class="flex flex-row items-center">
      <Header />
    </div>

    <div class="flex flex-col gap-4 p-4">
      <Searcher :filterFunction="filterCharacters" @search="searchItems"/>
      <transition-group name="fade" tag="div" class="flex flex-col gap-2">
        <template v-for="(character, index) in filteredCharacters" :key="index">
          <div class="card rounded-xl px-2">
            <div class="flex flex-row h-30 items-center gap-14 transition duration-700 hover:border hover:border-yellow-400" @click="goToCharacterCard(character)">
                <img class="character_icon rounded-xl" :src="character.image_url" />
              <div class="flex flex-col">
                <div class="text-2xl">{{ character.name }}</div>
              </div>
            </div>
          </div>
        </template>
      </transition-group>
    </div>
  </div>
  <Loader v-else />
</template>

<script>
import Searcher from '../components/Searcher.vue';
import Header from '../components/Header.vue';
import Loader from '../components/Loader.vue';
import axios from 'axios';

export default {
  components: { Header, Loader, Searcher },
  data() {
    return {
      characters: [],
      filteredCharacters: []
    }
  },
  methods: {
    filterCharacters(item, searchQuery) {
      return item.name.toLowerCase().includes(searchQuery.toLowerCase())
    },
    searchItems(searchQuery) {
      if (typeof searchQuery !== 'string') return;
      this.filteredCharacters = this.characters.filter(item => this.filterCharacters(item, searchQuery))
    },
    goToBack() {
      this.$router.push('/');
    },
    goToCharacterCard(character) {
      this.$router.push({ name: 'PersonCard', params: { id: character.id } });
    },
    charactersList() {
            axios.get('https://api.deadlock-helper.selfdoor.ru/characters/')
                .then(response => {
                  this.characters = response.data
                  this.filteredCharacters = response.data
                })
                .catch(error => {
                console.error(error);
                // Обработка ошибки
                if (error.response.status === 404) {
                  router.push({ name: 'not-found' });
                } else {this.error = error.response.data;}
                });
            },
  },
  created() {
    this.charactersList();
  }
}
</script>